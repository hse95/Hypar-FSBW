...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -onlypos:30:-2.5:-1:50:+2.5:20 -vars:-all,+vel
...
${measuretool} -dirin ${diroutdata} -filexml ${dirout}/case.xml -savecsv ${dirout2}/MeasureSWL -points /scratch/gpfs/hse/DualSPHysics/ProjectX/points.txt -vars:-all -height
if [ $? -ne 0 ] ; then fail; fi
